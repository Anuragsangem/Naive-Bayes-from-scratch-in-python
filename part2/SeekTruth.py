...
def clean_function(data):
    ...
    return tranformed_data #this transformed data is cleaned to stopwords , unnecesary symbols etc 
...
def classifier(train_data, test_data):
    ...
    Prior_prob_Truthful = T_count/len(train_data['labels']) #0.5
    Prior_prob_Deceptive = D_count/len(train_data['labels']) #0.5
    ...
    no_of_truth_reviews = len(Truthful_train_data)
    no_of_deceptive_reviews = len(Deceptive_train_data)
    ...
    for w in bag_of_words_truthful:
        ...
        for sentence in cleaned_Truthful_train_data:
            if w in sentence:
                count+=1
        prob_of_truth = (count+1)/(no_of_truth_reviews) #We are adding 1 in the numerator to prevent 0 probability from occuring - smoothing - also Laplace smoothing can be used here
    ...
    for w in bag_of_words_deceptive:
        ...
        for sentence_Decept in cleaned_Deceptive_train_data:
            if w in sentence_Decept:
                count_decept+=1
        prob_of_decept = (count_decept+1)/(no_of_deceptive_reviews) #We are adding 1 in the numerator to prevent 0 probability from occuring - smoothing - also Laplace smoothing can be used here
    ...
    for sentence in clean_ip:
        ...
        prob_input_is_deceptive=0.5 #This is the prior probability we calculated in the beggining - Prior_prob_Truthful
        prob_input_is_truth=0.5 #This is the prior probability we calculated beggining - Prior_prob_deceptive
        for i in words:
            if(len(i)>3): #doing this coz words like wi-fi are getting split as 2 seperate words which is not necessary hence Filtering out all words of length <3
    ...
    return op_list #this list has the predictions of out test_data
